...
- **App 01 (TaskFlow) frontend**: In progress. React 19 + Vite 8 + React Router v7 framework mode (SSR enabled). Entry files: `entry.client.tsx` (hydration with `startTransition`), `entry.server.tsx` (streaming SSR via `renderToPipeableStream`), `root.tsx` (Layout + Outlet + shared `ErrorBoundary` handling `isRouteErrorResponse`, `Error`, and unknown). Routes in `routes.ts` using `route()`/`index()` helpers. Pages: `HomePage.tsx` (loader fetches boards, renders `BoardList`; `action` handles create-board submissions via `useFetcher`, validates with Zod `CreateBoardSchema`, returns `ActionResult<Board>` discriminated union, catches thrown errors via `toActionError`), `BoardPage.tsx` (loader fetches board/columns/tasks in parallel via `Promise.all`, throws `data('...', { status: 404 })` on missing resources). API layer under `src/api/` (`client.ts` with typed `get`/`post`/`put`/`patch`/`del` wrappers around `fetch`; throws `HttpError` on `!response.ok` by parsing backend's `{ code, message, details }` error shape). Error-handling library under `src/lib/errors/`: `types.ts` (`ActionError` with optional `formError`/`fieldErrors`, `ActionResult<T>` discriminated union on `ok`, structural `ZodErrorLike`/`ZodIssueLike` for Zod-version independence), `httpError.ts` (`HttpError` base class + `BadRequestError`/`NotFoundError` subclasses mirroring backend), `toActionError.ts` (normalizes any `unknown` thrown value to `ActionError`), `zodErrorToActionError.ts` (converts `ZodError.issues` to `ActionError` with first-wins per-field, empty-path issues routed to `formError`). Frontend Zod schemas under `src/schemas/` (mirror backend validation). Types under `src/types/` mirror backend DTOs (`Board`, `Column`, `Task`, `Subtask`, `Label`, `TaskLabel`, `Snapshot`, `Filter` + `Create*Dto`/`Update*Dto`). Components: `BoardList`, `BoardView`, `ColumnView`, `TaskTile` (use `.toSorted()` for non-mutating sort), `Modal` (conditionally rendered so `useFetcher` state resets on close/reopen via unmount), `CreateBoardForm` (uses `fetcher.Form` + `useFetcher<ActionResult<Board>>()`, renders `formError`/`fieldErrors.name`, disables submit while `fetcher.state !== 'idle'`, auto-closes modal on `result?.ok` via `useEffect`). Styling: Tailwind CSS v4 + shadcn/ui (Radix) + Oxanium font. Path alias `@/` configured. Kanban UI, remaining forms/actions (column/task CRUD), clone/undo-redo/filter features still needed.
